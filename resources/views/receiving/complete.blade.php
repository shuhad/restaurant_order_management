@extends('app')
@section('content')
{!! Html::script('js/angular.min.js', array('type' => 'text/javascript')) !!}
{!! Html::script('js/app.js', array('type' => 'text/javascript')) !!}
<style>
table td {
    border-top: none !important;
}
</style>
<div class="container-fluid">
   <div class="row">
        <div class="col-md-12" style="text-align:center">
            <img src="images/logo.jpg"/>            
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            {{trans('receiving.supplier')}}: {{ $receivings->supplier->company_name}}<br />
            {{trans('receiving.receiving_id')}}: RECV{{$receivingItemsData->receiving_id}}<br />
        </div>
    </div>
    <div class="row">
<?php $mytotalprice=0;?>        
        <div class="col-md-12">
            <div class="table-responsive">
           <table class="table table-bordered">
                <tr>
                    <th>{{trans('receiving.item')}}</th>
                    <th>{{trans('receiving.price')}}</th>
                    <th>{{trans('receiving.qty')}}</th>
                    <th>{{trans('receiving.total')}}</th>
                </tr>
                @foreach($receivingItems as $value)
                <tr>
                    <td>{{$value->item->item_name}}</td>
                    <td>{{$value->cost_price}}</td>
                    <td>{{$value->quantity}}</td>
                    <td>{{$value->total_cost}}</td>
<?php $mytotalprice+=$value->total_cost;
 ?>                    
                </tr>
                @endforeach
                <tr>
                    <td colspan="3" align="right">Sub Total :</td>
                    <td>{{number_format($mytotalprice,2)}}</td>
                </tr>
                <tr>
                    <td colspan="3" align="right">Amount Paid  :</td>
                    <td>{{number_format($receivings->paidAmount,2)}}
</td>
                </tr>
                <tr>
                    <td colspan="3" align="right">Total Due : </td>
                    <td><?php $paidAmount=$mytotalprice-$receivings->paidAmount; ?>
                        {{number_format($paidAmount,2)}}</td>
                </tr>
            </table>
        </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            {{trans('receiving.payment_type')}}: {{$receivings->payment_type}}
        </div>
    </div>
    <hr class="hidden-print"/>
    <div class="row">
        <div class="col-md-8">
            &nbsp;
        </div>
        <div class="col-md-2">
            <button type="button" onclick="printInvoice()" class="btn btn-info pull-right hidden-print">{{trans('receiving.print')}}</button> 
        </div>
        <div class="col-md-2">
            <a href="{{ url('/receivings') }}" type="button" class="btn btn-info pull-right hidden-print">{{trans('receiving.new_receiving')}}</a>
        </div>
    </div>
</div>
<address align="center" style="text-align: center;">
    Elegant Shopping Mall, Zindabazar,Sylhet 3100<br/>
    Phone : 01791-112233 | facebook.com/teamruku | www.teamruku.com

</address>
<script>
function printInvoice() {
    window.print();
}
</script>
@endsection